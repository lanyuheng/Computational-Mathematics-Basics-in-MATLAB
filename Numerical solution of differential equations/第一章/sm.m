...
for i=2:n
    ...
    while abs(uc2-uc1)>delta
        uc1=uc2;
        uc2=u(i-1)+(h/2)*(4*t(i-1)*u(i-1)^(1/2)+4*t(i)*uc1^(1/2));
    end
    u(i)=uc2;
end
time3=toc;
...
tic;
for i=2:n
    uc1=u(i-1)+h*4*t(i-1)*(u(i-1)^(1/2));
    u(i)=u(i-1)+(h/2)*(4*t(i-1)*u(i-1)^(1/2)+4*t(i)*uc1^(1/2));
end
time4=toc;
NumU4=u;
...
tic;
for i=3:n
    u(i)=(1+a)*u(i-1)-a*u(i-2)+(1/2)*h*((3-a)*4*t(i-1)*(u(i-1)^(1/2))-(1+a)*4*t(i-2)*(u(i-2)^(1/2)));
end
time5=toc;
NumU5=u;
...
for i=3:n
    ...
    while abs(uc2-uc1)>delta
        uc1=uc2;
        uc2=u(i-2)+(h/3)*(4*t(i-2)*u(i-2)^(1/2)+4*4*t(i-1)*u(i-1)^(1/2)+4*t(i)*uc1^(1/2));
    end
    u(i)=uc2;
end
time7=toc;
...
tic;
for i=2:n
    k1=4*t(i-1)*u(i-1)^(1/2);
    k2=4*(t(i-1)+h/2)*(u(i-1)+(h/2)*k1)^(1/2);
    k3=4*(t(i-1)+h/2)*(u(i-1)+(h/2)*k2)^(1/2);
    k4=4*(t(i-1)+h)*(u(i-1)+h*k3)^(1/2);
